...
export default class BatchProgressIndicator extends LightningElement {
    ...
    get batchClassoptions(){
        return [{label: 'NotifyInactiveUsersBatchManagerNotify', value: 'NotifyInactiveUsersBatchManagerNotify'}];
    }
    ...
    handleExecuteBatch() {
        if(!this.batchClassName){
            this.showNotification('Batch Execution', 'Please Enter Batch Class Name','Warning');
        }else if(!this.batchSize){
            this.showNotification('Batch Execution', 'Please Enter Batch Size','Warning');
        }else{
